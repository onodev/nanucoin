...
class TxViewDelegate : public QAbstractItemDelegate
{
    Q_OBJECT
public:
    TxViewDelegate() : QAbstractItemDelegate(), unit(BitcoinUnits::NNC)
    {
    }
...
void OverviewPage::setWalletModel(WalletModel* model)
{
    ...
    // update the display unit, to not use the default ("NNC")
    updateDisplayUnit();
}
...
void OverviewPage::updateObfuscationProgress()
{
    ...
    QString strAnonymizeNanuCoinAmount = BitcoinUnits::formatHtmlWithUnit(nDisplayUnit, nAnonymizeNanuCoinAmount * COIN, false, BitcoinUnits::separatorAlways);
    ...
    if (currentBalance == 0) {
        ...
        strAnonymizeNanuCoinAmount = strAnonymizeNanuCoinAmount.remove(strAnonymizeNanuCoinAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizeNanuCoinAmount + " / " + tr("%n Rounds", "", nObfuscationRounds);
    ...
    if (nMaxToAnonymize > nAnonymizeNanuCoinAmount * COIN) nMaxToAnonymize = nAnonymizeNanuCoinAmount * COIN;

    if (nMaxToAnonymize == 0) return;

    if (nMaxToAnonymize >= nAnonymizeNanuCoinAmount * COIN) {
        ui->labelAmountRounds->setToolTip(tr("Found enough compatible inputs to anonymize %1")
                                              .arg(strAnonymizeNanuCoinAmount));
        strAnonymizeNanuCoinAmount = strAnonymizeNanuCoinAmount.remove(strAnonymizeNanuCoinAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizeNanuCoinAmount + " / " + tr("%n Rounds", "", nObfuscationRounds);
    } else {
