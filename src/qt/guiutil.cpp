...
void setupAddressWidget(QValidatedLineEdit* widget, QWidget* parent)
{
    ...
    widget->setPlaceholderText(QObject::tr("Enter a NanuCoin address (e.g. %1)").arg("AWSbBnzmNkjDVaYHX7vkL1MqD96pRYWtZo"));
#endif
...
{
    // return if URI is not valid or is no NanuCoin: URI
...
{
    // Convert nanucoin:// to nanucoin:
    //
    //    Cannot handle this later, because nanucoin:// will cause Qt to see the part after // as host,
...
void openConfigfile()
{
    ...
    /* Open nanucoin.conf with the associated application */
...
{
    return GetSpecialFolderPath(CSIDL_STARTUP) / "NanuCoin.lnk";
}

bool GetStartOnSystemStartup()
{
    // check for NanuCoin.lnk
...
{
    return GetAutostartDir() / "nanucoin.desktop";
}
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    if (!fAutoStart)
        ...
        if (!optionFile.good())
            return false;
        // Write a nanucoin.desktop file to the autostart directory:
        optionFile << "[Desktop Entry]\n";
        optionFile << "Type=Application\n";
        optionFile << "Name=NanuCoin\n";
...
LSSharedFileListItemRef findStartupItemInList(LSSharedFileListRef list, CFURLRef findUrl)
{
    // loop through the list of startup items and try to find the nanucoin app
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    if (fAutoStart && !foundItem) {
        // add nanucoin app to startup item list
