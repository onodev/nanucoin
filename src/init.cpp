...
std::string HelpMessage(HelpMessageMode mode)
{
...
#ifndef WIN32
    strUsage += HelpMessageOpt("-pid=<file>", strprintf(_("Specify pid file (default: %s)"), "nanucoind.pid"));
#endif
    ...
    string debugCategories = "addrman, alert, bench, coindb, db, lock, rand, rpc, selectcoins, mempool, net, nanucoin, (obfuscation, swifttx, masternode, mnpayments, mnbudget)"; // Don't translate these and qt below
    if (mode == HMM_BITCOIN_QT)
        debugCategories += ", qt";
    ...
    strUsage += HelpMessageOpt("-litemode=<n>", strprintf(_("Disable all NNC specific functionality (Masternodes, Obfuscation, SwiftTX, Budgeting) (0-1, default: %u)"), 0));
    ...
    strUsage += HelpMessageOpt("-anonymizenanucoinamount=<n>", strprintf(_("Keep N PIV anonymized (default: %u)"), 0));
...
std::string LicenseInfo()
{
    return FormatParagraph(strprintf(_("Copyright (C) 2009-%i The Bitcoin Core Developers"), COPYRIGHT_YEAR)) + "\n" +
           ...
           "\n" +
           FormatParagraph(strprintf(_("Copyright (C) 2015-%i The NanuCoin Core Developers"), COPYRIGHT_YEAR)) + "\n" +
           "\n" +
...
/** Sanity checks
 *  Ensure that NanuCoin is running in a usable environment with all
...
#define PROCESS_DEP_ENABLE 0x00000001
    ...
    // Sanity check
    if (!InitSanityCheck())
        return InitError(_("Initialization sanity check failed. NanuCoin Core is shutting down."));
...
#endif
    // Make sure only a single NanuCoin process is using the data directory.
    ...
    if (!lock.timed_lock(boost::get_system_time() + boost::posix_time::seconds(10)))
        return InitError(strprintf(_("Cannot obtain a lock on data directory %s. NanuCoin Core is probably already running."), strDataDir));
...
// ********************************************************* Step 8: load wallet
    ...
    nAnonymizeNanuCoinAmount = GetArg("-anonymizenanucoinamount", 0);
    if (nAnonymizeNanuCoinAmount > 999999) nAnonymizeNanuCoinAmount = 999999;
    if (nAnonymizeNanuCoinAmount < 2) nAnonymizeNanuCoinAmount = 2;
    ...
    LogPrintf("Anonymize NanuCoin Amount %d\n", nAnonymizeNanuCoinAmount);
    ...
    /* Denominations
       ...
       For example:
       1NNC+1000 == (.1NNC+100)*10
       10NNC+10000 == (1NNC+1000)*10
    */
